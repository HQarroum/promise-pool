...
class Pool {
   ...
   */
  enqueue(f) {
    return (Array.isArray(f) ? this.strategy.enqueueMany(f) : this.strategy.enqueue(f));
  }
  ...
  enqueueOnSameExecutor(array) {
    enforceArray(array);
    return (this.strategy.enqueueOnSameExecutor(array));
  }
  ...
  /**
   * @return a promise selected from the internal
   * promise pool.
   */
  promise() {
    return (this.strategy.promise());
  }
